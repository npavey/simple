﻿define(['eventManager', 'guard', 'requester/resourceLoader', 'constants'], function (
  eventManager,
  guard,
  resourceLoader,
  constants
  ...
  function Question(spec, _protected) {
    ...
    (this.loadFeedback = loadFeedback), (this.loadLearningContent = loadLearningContent);
    ...
    this.progress = function (data) {
      if (!this.affectProgress && !this.hasOwnProperty('isSurvey')) return 0;
    ...
    this.submitAnswer = function () {
      ...
      if (!this.affectProgress && !this.hasOwnProperty('isSurvey')) return;
  ...
  function loadContent(items) {
    ...
    _.each(items, function (item) {
      if (typeof item.content === typeof undefined) {
        promises.push(
          loadPage(item.contentUrl).then(function (content) {
  ...
  function loadPage(contentUrl) {
    return resourceLoader.getLocalResource({
      url: contentUrl
    });
  }
});
