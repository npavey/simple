define([
  ...
  router.on("router:route:activating").then(function(newView) {
    if (
      ...
      var isNotAccountModule =
        newView &&
        newView.__moduleId__.slice(0, newView.__moduleId__.indexOf("/")) !==
          "account";
    ...
    if (
      newView &&
      currentView &&
      newView.__moduleId__ === currentView.__moduleId__
    ) {
      return;
    }
  ...
  function activate() {
    ...
    function initxApi() {
      return xApiInitializer.initialize(
        templateSettings.xApi,
        templateSettings.nps
      );
    }
    ...
    function initializeProgressProvider() {
      if (
        !publishModeProvider.isScormEnabled &&
        !publishModeProvider.isPreview &&
        !publishModeProvider.isReview
      ) {
  ...
  function viewSettings() {
    ...
    if (_.isObject(activeInstruction)) {
      settings.rootLinkEnabled =
        !!activeInstruction.config.rootLinkEnabled &&
        !router.isNavigationLocked();
      settings.exitButtonVisible =
        showExitButton(activeInstruction.config) &&
        !templateSettings.hideFinishActionButtons;
      settings.treeOfContentVisible =
        templateSettings.treeOfContent.enabled &&
        !!activeInstruction.config.displayTreeOfContent;
    }
  ...
  function showExitButton(activeInstructionConfig) {
    return (
      !!activeInstructionConfig.showExitButton ||
      (!!activeInstructionConfig.showExitButtonInReviewMode &&
        context.isInReviewAttemptMode())
    );
  }
});
